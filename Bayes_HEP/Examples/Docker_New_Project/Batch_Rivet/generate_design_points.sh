...
if [ "$DO_BATCH_RUN" = true ]; then
    ...
    rm -rf "$MAIN_DIR/Batch_Rivet/logs"
    mkdir -p "$MAIN_DIR/Batch_Rivet/logs"
    ...
    for ((j=0; j<TOTAL_EVENTS; j+=NEVENTS)); do
        ...
        for ((i=0; i<TOTAL_POINTS; i+=BATCH_SIZE)); do
            ...
            python "$MAIN_SCRIPT" "$start" "$end" \
                ...
                --Write_input_Rivet False \
                --Coll_System ${COLLISIONS} \
                > "$MAIN_DIR/logs/output_${start}_$l.log" \
                2> "$MAIN_DIR/logs/error_${start}_$l.log"
...
if [ "$DO_RIVET_MERGE" = true ]; then
    ...
    for ((i=0; i<TOTAL_POINTS; i+=BATCH_SIZE)); do
        ...
        python "$MAIN_SCRIPT" "$start" "$end" \
            ...
            --Write_input_Rivet False \
            --Coll_System ${COLLISIONS} \
            > "$MAIN_DIR/Batch_Rivet/logs/output_html_${start}.log" \
            2> "$MAIN_DIR/Batch_Rivet/logs/error_html_${start}.log"
